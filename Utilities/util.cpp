...
// Geometria 3d
/*
Resuelve si un rtiangulo esta dentro de otro
##################################################################
############################*+####################################
##########################*+*+####################################
########################*+*##+*###################################
...
{
    plane A(pts[0], pts[i], pts[j]);
    vector<pair<int, int>> conex = { {other[0], other[1]},{other[1], other[2]},{other[2], other[0]} };
    int cnt = 0;
    each(ii, conex) {
        int a = ii.f, b = ii.s;
    
        pair<RETCODE, pt> res = intersect(A, line(pts[a], pts[b]));
        RETCODE code = res.f;
        pt p = res.s;
    
        if (code == ONE && isinpol(p, { pts[0], pts[i], pts[j] })) cnt++;
    }
    if (cnt == 1) ans.pb({ i + 1, j + 1 });
}
